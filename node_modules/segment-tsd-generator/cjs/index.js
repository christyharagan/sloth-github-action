...
const json_to_ts_1 = require("./json_to_ts");
const JS_OPTIONS = {
...
async function to_ts(schema, name) {
    let s = await json_to_ts_1.compile(schema, name);
    return s.replace("export interface", "declare interface").replace(/\[k\:\ string\]\:\ any/g, '');
}
async function default_1(tracking_plan) {
...
`;
    ...
    if (tracking_plan.rules.events && tracking_plan.rules.events.length > 0) {
        ...
        s += `
`;
        s += `declare type SegmentTrackProtocol<E extends SegmentEvents> = ${tracking_plan.rules.events.reduce((s, e) => `${s}E extends '${escapeString(e.name)}' ? ${escapeString(e.name).replace(/[ ]/g, '')} : `, '')}`;
        s += `never;
