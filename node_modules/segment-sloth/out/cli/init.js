...
exports.init = void 0;
const i = __importStar(require("inquirer"));
...
async function copy_file_with_substitution(src_path, dest_path, answers) {
...
async function init(save_only_to_file, settings_dir, overwrite_fn, advanced) {
    let file = settings_1.get_settings_file(settings_dir);
    if (settings_dir && !save_only_to_file && fs.existsSync(file)) {
        settings_1.load_settings(file).then(async (settings) => {
        ...
        if (!advanced) {
            if (await ngrok_1.ngrok_installed()) {
        ...
        function tracking_plan_prompt() {
            if (answers.access_token && answers.work_slug && answers.language == 'typescript') {
                config_api_1.listAllTrackingPlans(answers.access_token, answers.work_slug).then(tps => {
                    prompts.next({
                        name: 'tp_name',
        ...
        i.prompt(prompts).ui.process.subscribe(answer => {
            ...
            if (!save_only_to_file) {
                let saved_answers = Object.assign({}, answers);
                if (!answers.access_token_is_saved) {
                    delete saved_answers.access_token;
                }
                settings_1.save_settings(file, saved_answers);
...
exports.init = init;
async function deploy_function_first_time(answers, can_overwrite) {
    if (answers.work_slug && answers.work_id && answers.access_token) {
        ...
        let id = await deployer_1.build_function(answers.fn_name, answers.fn_type == 'source', answers.work_slug, answers.work_id, c, answers.access_token, undefined, can_overwrite ? undefined : true);
        if (!id) {
            return false;
        }
        answers.fn_id = id;
...
function create_project(answers) {
    ...
    if (answers.language == 'javascript') {
        fs.copy(JS_PROJECT_DIR, cwd).then(() => {
            child_process_1.exec('npm i', (error) => {
                if (error) {
                    console.error(error);
                }
            });
    ...
    if (answers.tp_name && answers.tp_name !== 'none' && answers.access_token && answers.work_slug) {
        ...
        fs.copyFile(path.join(TS_TYPINGS_DIR, 'protocol.d.ts'), generate_protocol_1.get_protocol_filename());
    }
}
